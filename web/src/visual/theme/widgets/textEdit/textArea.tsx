...
class TextArea extends Component<TextEditProps, TextEditState> {
	...
	render() {
		const { placeholder, dirty, className, disable, value, autoFocus=false, edit, rows, cols } = this.props;

		const { tmpValue } = this.state;

		let cn = 'text-area position-relative';
		...
		return (
			<div className={cn}>
				<textarea className={'position-absolute h-100 w-100'}
						  ref={this.inputRef}
